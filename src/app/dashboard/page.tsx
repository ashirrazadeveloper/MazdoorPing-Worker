...
export default function DashboardPage() {
  ...
  const categoryInfo = WORKER_CATEGORIES[worker.category]
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-green-100/80 text-[11px] mt-0.5">
                  {categoryInfo.icon} {categoryInfo.en} ({categoryInfo.ur}) · {worker.area}, {worker.city}
                </p>
              </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Card className="overflow-hidden border-border/60 hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <span className="text-[11px] text-muted-foreground font-medium">Active Jobs</span>
              </div>
          ...
          <Card className="overflow-hidden border-border/60 hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              ...
              <p className="text-xl font-bold text-foreground">{worker.rating} <span className="text-sm text-muted-foreground font-normal">({worker.total_reviews})</span></p>
            </CardContent>
          </Card>
          <Card className="overflow-hidden border-border/60 hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              ...
              <p className="text-xl font-bold text-foreground">{formatPKR(worker.wallet_balance)}</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center p-3 bg-green-50 rounded-xl">
                ...
                <p className="text-sm font-bold text-green-700 mt-1">{formatPKR(27000)}</p>
              </div>
              <div className="text-center p-3 bg-emerald-50 rounded-xl">
                <p className="text-[10px] text-muted-foreground font-medium">This Month</p>
                <p className="text-sm font-bold text-emerald-700 mt-1">{formatPKR(86500)}</p>
              </div>
              <div className="text-center p-3 bg-primary/5 rounded-xl">
                <p className="text-[10px] text-muted-foreground font-medium">All Time</p>
                <p className="text-sm font-bold text-primary mt-1">{formatPKR(worker.total_earnings)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
