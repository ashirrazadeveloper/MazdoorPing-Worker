...
export default function JobCard({ job, showEmployer = true }: JobCardProps) {
  const categoryIcon = job.category?.icon || '🔧'
  const categoryName = job.category?.name || 'Job'
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">{categoryName} {job.category?.name_ur && `· ${job.category.name_ur}`}</p>
            </div>
          </div>
          {job.urgency !== 'normal' && (
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-bold text-primary text-sm">
              ...
              {job.budget_max != null && job.budget_min != null && job.budget_max > job.budget_min && ` - ${formatPKR(job.budget_max)}`}
            </span>
          </div>
          {showEmployer && job.employer && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              {job.employer.rating != null && (
                <Star className="h-3 w-3 fill-yellow-400 text-yellow-400" />
              )}
              <span className="bg-muted/70 px-2 py-0.5 rounded-md">{job.employer.full_name || 'Employer'}</span>
            </div>
          )}
        </div>
      </Card>
